...
log(abundance) \~ treatment + f_year + (1 + treatment \| plot_name)
:::
...
The log-transformed response with treatment as a fixed effect, and a random intercept and a random slope for treatment within plot name emerged as the best initial model based on AIC values and diagnostic plots.
...
```{r}
# lmer_2: lmer(abundance ~ treatment + (1 + treatment | plot_name))
model_init_abun_nat_sqrt <- lmer(
  value_sqrt ~ treatment + (1 + treatment | plot_name),
  data = abun, subset = met_sub == "abun_nat", REML = FALSE
)
fxn_model_review(model_init_abun_nat_sqrt, abun_nat)
...
The addition of sampling year as a fixed effect emerged as the best model after fixed effects selection based on AIC values and diagnostic plots.
...
This model and the diagnostics were previously described in the *Fixed effects selection* section.
...
# View the summary table
fixed_summary_table <- models_fixed_abun_frb$summary_table %>%
  as_tibble() %>%
  dplyr::select(model_name, terms_count, aic) %>%
  dplyr::mutate(delta = round(aic - min(aic), 1))
...
# View the best model 
best_fixed_model <- models_fixed_abun_frb$best_model$formula
