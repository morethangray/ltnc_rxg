...
#' 3. Computes back-transformed marginal means using marginaleffects
#' 4. Joins with lookup tables for additional context
#'
#' @importFrom marginaleffects marginalmeans
...
fxn_summarize_marginal_means <- function(index_model_name, lookup_tables) {
  ...
  marginal_means <- 
    marginaleffects::marginalmeans(index_model, "treatment") %>%
    broom::tidy() %>%
    tibble::as_tibble() %>%
    janitor::clean_names() %>%
...
#
# Not run: To get package version marginaleffects_0.5.0 ----
